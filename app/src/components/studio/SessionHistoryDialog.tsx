                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        {formatDateTime(session.date, locale)}
                      </p>
                    </div>
                    <p className="text-right text-lg font-semibold tabular-nums text-foreground">
                      {formatCredits(session.credits, locale)} cr
                    </p>
                  </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
                    <div className="rounded-xl bg-background/45 px-3 py-2">
                      <p className="flex items-center gap-1.5 text-[10px] uppercase tracking-wide text-muted-foreground">
                        <Clock3 className="size-3" /> {t('studio.duration')}
                      </p>
                    ...
                    <div className="rounded-xl bg-background/45 px-3 py-2">
                      <p className="flex items-center gap-1.5 text-[10px] uppercase tracking-wide text-muted-foreground">
                        <Coins className="size-3" /> {t('studio.rate')}
                      </p>
                      <p className="mt-1 text-sm font-medium tabular-nums text-foreground">
                        {t('studio.ratePerSecond', { rate: session.rate })}
                      </p>
                    </div>
                    <div className="col-span-2 rounded-xl bg-background/45 px-3 py-2 sm:col-span-1">
                      <p className="text-[10px] uppercase tracking-wide text-muted-foreground">{t('studio.reason')}</p>
                      <p className="mt-1 truncate text-sm font-medium text-foreground">
                        {session.reason || (session.status === 'active' ? t('studio.liveNow') : t('common.completed'))}
                      </p>
                    </div>
                  </div>

                  <p className="mt-3 truncate font-mono text-[10px] text-muted-foreground/70">
                    {t('studio.providerLine', {
                      providerId: session.providerSessionId || t('studio.notAssigned'),
                      id: session.id,
                    })}
                  </p>
                </article>
